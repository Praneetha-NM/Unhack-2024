    ...
    while n<=total:
        for machine in step_machines:
            if machine in free_machines:
                machine_execution(machine,param_range,param,result,machine_count,wafer,count,machines,process)
                n+=1
                break
if __name__ == "__main__":
    ...
    count=0
    for wafer in wafers:
        for process in wafer["processing_times"].keys():
            for i in range(wafer["quantity"]): 
                threading.Thread(target=step, args=(steps,machines,wafers,dependency_graph,machines_for_step,free_machines,count,initial_param,result,total_no_wafers,1,machine_count))
    print(result)
